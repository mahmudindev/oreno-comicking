...
import type { NewTagType, ParameterTagType } from '$lib/model';
...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	try {
		...
		const result = await listTagType(
			{ user, database },
			{ limit, offset, page, orderBys, ...param }
		);
		const totalCount = await countTagType({ user, database }, { ...param });
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const result = await addTagType({ user, database }, data);

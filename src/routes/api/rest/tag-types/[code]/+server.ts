...
import type { SetTagType } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import { deleteTagTypeByKey, getTagTypeByKey, updateTagTypeByKey } from '$lib/server/service';
...
export const GET: RequestHandler = async ({ params, request }) => {
	try {
		...
		const result = await getTagTypeByKey({ user, database }, params.code);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const result = await updateTagTypeByKey({ user, database }, params.code, data);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	try {
		...
		await deleteTagTypeByKey({ user, database }, params.code);

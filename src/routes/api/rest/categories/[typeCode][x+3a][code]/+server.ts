...
import type { SetCategory } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import { deleteCategoryByKey, getCategoryByKey, updateCategoryByKey } from '$lib/server/service';
...
export const GET: RequestHandler = async ({ params, request }) => {
	try {
		...
		const result = await getCategoryByKey({ user, database }, params.typeCode, params.code);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	try {
		...
		await deleteCategoryByKey({ user, database }, params.typeCode, params.code);

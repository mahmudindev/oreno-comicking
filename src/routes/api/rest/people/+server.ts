...
import type { NewPerson, ParameterPerson } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import { addPerson, countPerson, listPerson } from '$lib/server/service';
...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	try {
		...
		const totalCount = await countPerson({ user, database }, { ...param });
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const result = await addPerson({ user, database }, data);

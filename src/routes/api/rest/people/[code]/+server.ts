...
import type { SetPerson } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import { deletePersonByKey, getPersonByKey, updatePersonByKey } from '$lib/server/service';
...
export const GET: RequestHandler = async ({ params, request }) => {
    try {
        ...
        const result = await getPersonByKey({ user, database }, params.code);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
    ...
    try {
        ...
        const result = await updatePersonByKey({ user, database }, params.code, data);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
    try {
        ...
        await deletePersonByKey({ user, database }, params.code);

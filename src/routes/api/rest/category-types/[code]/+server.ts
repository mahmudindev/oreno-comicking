...
import type { SetCategoryType } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import {
	deleteCategoryTypeByKey,
	getCategoryTypeByKey,
	updateCategoryTypeByKey
...
export const GET: RequestHandler = async ({ params, request }) => {
	try {
		...
		const result = await getCategoryTypeByKey({ user, database }, params.code);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const result = await updateCategoryTypeByKey({ user, database }, params.code, data);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	try {
		...
		await deleteCategoryTypeByKey({ user, database }, params.code);

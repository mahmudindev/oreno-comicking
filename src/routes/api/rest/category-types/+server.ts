...
import type { NewCategoryType, ParameterCategoryType } from '$lib/model';
...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	const param: ParameterCategoryType = {
	...
	try {
		...
		const result = await listCategoryType(
			{ user, database },
			{ limit, offset, page, orderBys, ...param }
		);
		const totalCount = await countCategoryType({ user, database }, { ...param });
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const result = await addCategoryType({ user, database }, data);

...
import type { SetMagazine } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import { deleteMagazineByKey, getMagazineByKey, updateMagazineByKey } from '$lib/server/service';
...
export const GET: RequestHandler = async ({ params, request }) => {
    try {
        ...
        const result = await getMagazineByKey({ user, database }, params.code);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
    ...
    try {
        ...
        const result = await updateMagazineByKey({ user, database }, params.code, data);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
    try {
        ...
        await deleteMagazineByKey({ user, database }, params.code);

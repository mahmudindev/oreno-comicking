...
import type { NewMagazine, ParameterMagazine } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import { addMagazine, countMagazine, listMagazine } from '$lib/server/service';
...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	try {
		...
		const result = await listMagazine(
			{ user, database },
			{ limit, offset, page, orderBys, ...param }
		);
		const totalCount = await countMagazine({ user, database }, { ...param });
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const result = await addMagazine({ user, database }, data);

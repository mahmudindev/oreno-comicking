...
import type { NewComicAuthor, ParameterComicAuthor } from '$lib/model';
...
export const GET: RequestHandler = async ({ params, request, url }) => {
	...
	const param: ParameterComicAuthor = {
		criteriaComicCodes: [params.comicCode],
		criteriaPositionCodes: url.searchParams.getAll('positionCode'),
		criteriaPersonCodes: url.searchParams.getAll('personCode'),
        criteriaPositionCodePersonCodes: url.searchParams.getAll('positionCodePersonCode')
	};
	...
	try {
		...
		const totalCount = await countComicAuthor({ user, database }, { ...param });

...
import type { NewComicChapter } from '$lib/model';
...
export const GET: RequestHandler = async ({ params, request, url }) => {
	...
	try {
		...
		const result = await listComicChapter(
			{ user, database },
			{ limit, offset, page, criteria, orderBys }
		);
		const totalCount = await countComicChapter({ user, database }, { criteria });
...
export const POST: RequestHandler = async ({ params, request }) => {
	...
	switch (request.headers.get('Content-Type')) {
		...
		case 'application/x-www-form-urlencoded': {
			...
			data.number = toNumber(f.get('number')) ?? NaN;
	...
	try {
		...
		const result = await addComicChapter({ user, database }, data);

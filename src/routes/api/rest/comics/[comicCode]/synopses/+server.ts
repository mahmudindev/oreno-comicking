...
import type { NewComicSynopsis, ParameterComicSynopsis } from '$lib/model';
...
export const GET: RequestHandler = async ({ params, request, url }) => {
	...
	const param: ParameterComicSynopsis = {
	...
	try {
		...
		const result = await listComicSynopsis(
			{ user, database },
			{ limit, offset, page, orderBys, ...param }
		);
		const totalCount = await countComicSynopsis({ user, database }, { ...param });
...
export const POST: RequestHandler = async ({ params, request }) => {
	...
	switch (request.headers.get('Content-Type')) {
		...
		case 'application/x-www-form-urlencoded': {
			...
			data.languageLang = f.get('language_lang') as string;
			data.content = f.get('content') as string;
			data.source = (f.get('source') as string | null) ?? undefined;
			break;
		}
		default:
	...
	try {
		...
		const result = await addComicSynopsis({ user, database }, data);

		return json(result, {
			headers: {
				Location: new URL(request.url).pathname + '/' + result.ulid,
				'X-Content-Type-Options': 'nosniff'
			}
		});
	} catch (e) {

...
import type { SetComicSynopsis } from '$lib/model';
import { json } from '@sveltejs/kit';
import { getDatabase, getUser } from '$lib/server/context';
import {
	deleteComicSynopsisByKey,
	getComicSynopsisByKey,
	updateComicSynopsisByKey
...
export const GET: RequestHandler = async ({ params, request }) => {
	try {
		...
		const result = await getComicSynopsisByKey({ user, database }, params.comicCode, params.ulid);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const result = await updateComicSynopsisByKey(
			{ user, database },
			params.comicCode,
			params.ulid,
			data
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	try {
		...
		await deleteComicSynopsisByKey({ user, database }, params.comicCode, params.ulid);

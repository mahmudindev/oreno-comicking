...
import type { NewComicExternal, ParameterComicExternal } from '$lib/model';
...
export const GET: RequestHandler = async ({ params, request, url }) => {
	...
	const param: ParameterComicExternal = {
		criteriaComicCodes: [params.comicCode],
		criteriaLinkWebsiteHosts: url.searchParams.getAll('linkWebsiteHost'),
		criteriaLinkRelativeReferences: url.searchParams.getAll('linkRelativeReference'),
		criteriaLinkHREFs: url.searchParams.getAll('linkHREF')
	};
	...
	try {
		...
		const totalCount = await countComicExternal({ user, database }, { ...param });

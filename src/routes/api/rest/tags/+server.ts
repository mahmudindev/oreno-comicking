...
import type { NewTag, ParameterTag } from '$lib/model';
...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	try {
		...
		const result = await listTag({ user, database }, { limit, offset, page, orderBys, ...param });
		const totalCount = await countTag({ user, database }, { ...param });
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const result = await addTag({ user, database }, data);

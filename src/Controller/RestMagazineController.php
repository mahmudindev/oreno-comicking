...
#[Routing\Route(
    path: '/api/rest/magazines',
    name: 'rest_magazine_'
)]
class RestMagazineController extends AbstractController
{
    ...
    public function list(
        ...
        $result = $this->magazineRepository->findByCustom($criteria, $orderBy, $limit, $offset);

        $headers = [];
        $headers['X-Total-Count'] = $this->magazineRepository->countCustom($criteria);
        $headers['X-Pagination-Limit'] = $limit;

        $response = $this->json($result, Response::HTTP_OK, $headers, ['groups' => ['magazine']]);
    ...
    public function post(
        ...
        $headers = [];
        $headers['Location'] = $this->generateUrl('rest_magazine_get', ['code' => $result->getCode()]);

        return $this->json($result, Response::HTTP_CREATED, $headers, ['groups' => ['magazine']]);
    }
    ...
    public function get(
        ...
        $result = $this->magazineRepository->findOneBy(['code' => $code]);
        if (!$result) throw new NotFoundHttpException('Magazine not found.');

        $response = $this->json($result, Response::HTTP_OK, [], ['groups' => ['magazine']]);
    ...
    public function patch(
        ...
        return $this->json($result, Response::HTTP_OK, $headers, ['groups' => ['magazine']]);
    }

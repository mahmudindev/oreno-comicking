...
#[Routing\Route(
    path: '/api/rest/characters',
    name: 'rest_character_'
)]
class RestCharacterController extends AbstractController
{
    ...
    public function list(
        ...
        $result = $this->characterRepository->findByCustom($criteria, $orderBy, $limit, $offset);

        $headers = [];
        $headers['X-Total-Count'] = $this->characterRepository->countCustom($criteria);
        $headers['X-Pagination-Limit'] = $limit;

        $response = $this->json($result, Response::HTTP_OK, $headers, ['groups' => ['character']]);
    ...
    public function post(
        ...
        $headers = [];
        $headers['Location'] = $this->generateUrl('rest_character_get', ['code' => $result->getCode()]);

        return $this->json($result, Response::HTTP_CREATED, $headers, ['groups' => ['character']]);
    }
    ...
    public function get(
        ...
        $result = $this->characterRepository->findOneBy(['code' => $code]);
        if (!$result) throw new NotFoundHttpException('Character not found.');

        $response = $this->json($result, Response::HTTP_OK, [], ['groups' => ['character']]);
    ...
    public function patch(
        ...
        return $this->json($result, Response::HTTP_OK, $headers, ['groups' => ['character']]);
    }

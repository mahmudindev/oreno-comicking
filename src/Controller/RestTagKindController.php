...
class RestTagKindController extends AbstractController
{
    ...
    public function list(
        ...
        $result = $this->tagKindRepository->findByCustom($criteria, $orderBy, $limit, $offset);

        $headers = [];
        $headers['X-Total-Count'] = $this->tagKindRepository->countCustom($criteria);
        $headers['X-Pagination-Limit'] = $limit;

        $response = $this->json($result, Response::HTTP_OK, $headers, ['groups' => ['tagType']]);
    ...
    public function post(
        ...
        $headers = [];
        $headers['Location'] = $this->generateUrl('rest_tagtype_get', ['code' => $result->getCode()]);

        return $this->json($result, Response::HTTP_CREATED, $headers, ['groups' => ['tagType']]);
    }
    ...
    public function get(
        ...
        if (!$result) throw new NotFoundHttpException('Tag Type not found.');

        $response = $this->json($result, Response::HTTP_OK, [], ['groups' => ['tagType']]);
    ...
    public function patch(
        ...
        return $this->json($result, Response::HTTP_OK, $headers, ['groups' => ['tagType']]);
    }
